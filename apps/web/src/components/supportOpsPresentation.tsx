...
export function toneFor(status: string): string {
  if (["completed", "approved", "verified", "resolved", "resolve", "desired_state_observed", "applied_acknowledged", "healthy"].includes(status)) return "success";
  if (["running", "investigating"].includes(status)) return "progress";
  if (["outcome_unknown", "unknown", "not_verified", "inconclusive", "keep_open", "high", "medium", "insufficient_evidence", "pending", "awaiting_approval", "rejected", "undesired_state_observed", "critical"].includes(status)) return "warning";
